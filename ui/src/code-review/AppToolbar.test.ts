...
describe('AppToolbar', () => {
  ...
  it("Phase 28: Test C — AppToolbarProps includes onReviewSent:", () => {
    expect(source).toContain('onReviewSent:')
  })

  it("Phase 28: Test D — source contains 'Send Review' literal (button label)", () => {
    expect(source).toContain("'Send Review'")
  })

  it('Phase 28: Test E — source contains var(--color-accent-approve) (Send Review background)', () => {
    expect(source).toContain('var(--color-accent-approve)')
  })

  it("Phase 28: Test F — source contains import from './buildCodeReviewPayload'", () => {
    expect(source).toContain("from './buildCodeReviewPayload'")
  })

  it("Phase 28: Test G — source contains import from '../shared/connectivity' for ConnectivityStatus", () => {
    expect(source).toContain("from '../shared/connectivity'")
  })

  it("Phase 28: Test H — source contains import for CodeReviewSubmitPopover from './CodeReviewSubmitPopover'", () => {
    expect(source).toContain("CodeReviewSubmitPopover from './CodeReviewSubmitPopover'")
  })

  it("Phase 28: Test I — source contains SubmitState union literal 'confirmed'", () => {
    expect(source).toContain("'confirmed'")
  })

  it("Phase 28: Test J — source contains 'clipboard_confirmed' and 'clipboard_error'", () => {
    expect(source).toContain("'clipboard_confirmed'")
    expect(source).toContain("'clipboard_error'")
  })

  it('Phase 28: Test K — source contains window.close() (auto-close after confirmed)', () => {
    expect(source).toContain('window.close()')
  })

  it('Phase 28: Test L — source contains 3000 (clipboard_confirmed auto-reset)', () => {
    expect(source).toContain('3000')
  })

  it('Phase 28: Test M — source contains 500 (confirmed auto-close delay)', () => {
    expect(source).toContain('500')
  })

  it('Phase 28: Test N — source contains shouldUseClipboard(connectivity) (offline branch entry)', () => {
    expect(source).toContain('shouldUseClipboard(connectivity)')
  })

  it("Phase 28: Test O — source contains buildReviewPayload (the serializer)", () => {
    expect(source).toContain('buildReviewPayload')
  })

  it("Phase 28: Test P — source contains fetch('/api/decide' (the POST endpoint)", () => {
    expect(source).toContain("fetch('/api/decide'")
  })

  it("Phase 28: Test Q — source contains 'Review sent' (confirmed status message)", () => {
    expect(source).toContain("'Review sent'")
  })

  it("Phase 28: Test R — source contains 'Copied to clipboard' AND 'Paste into your Claude conversation.'", () => {
    expect(source).toContain("'Copied to clipboard'")
    expect(source).toContain("'Paste into your Claude conversation.'")
  })

  it("Phase 28: Test S — source contains 'Clipboard write failed' AND 'Dismiss' AND JSON payload aria-label", () => {
