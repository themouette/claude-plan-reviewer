...
/// Content of the annotate.md slash command file.
/// Shared between install and update paths to keep them in sync.
pub(crate) const ANNOTATE_MD: &str = concat!(
    "---\n",
...
/// Content of the code-review.md slash command file.
/// Shared between install and update paths to keep them in sync.
pub(crate) const CODE_REVIEW_MD: &str = concat!(
    "---\n",
...
impl Integration for ClaudeIntegration {
    ...
    fn install(&self, ctx: &InstallContext) -> Result<(), String> {
        ...
        if let Err(e) = std::fs::write(&annotate_path, ANNOTATE_MD) {
        ...
        if let Err(e) = std::fs::write(&code_review_path, CODE_REVIEW_MD) {
            return Err(format!(
                "cannot write {}: {}",
